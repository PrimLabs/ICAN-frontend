...
export const CreateModal = ({bucket, open, setCreate, setList, setStatus,}: Props) => {
    ...
    const [val, setVal] = useState<any>({
        ...
        compute: 0,
        init: 0,
        controllers: [bucket, String(principal), '']
    });
    const handleFile = (e) => {
        //@ts-ignore
    ...
    const handleClick = async () => {
        ...
        val.controllers.forEach(e => {
            try {
                controllers.push(Principal.fromText(e))
            } catch (e) {
            }
        })
                            ...
                            <>
                                ...
                                <div>
                                    ...
                                    {val.controllers.map((v, index) => {
                                        return (
                                            <div key={index} className={"relative flex items-center mt-[1.125rem]"}>
                                                <Input
                                                    id="controllers"
                                                    onChange={(e) => changeControllers(e.target.value, index)}
                                                    placeholder={v}
                                                    value={v}
                                                />
                                                <div className={"absolute right-0.5 cursor-pointer"}
                                                     onClick={() => {
